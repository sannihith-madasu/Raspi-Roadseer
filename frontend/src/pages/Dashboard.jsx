...
import { AlertTriangle, MapPin, Cpu, Activity, Users } from "lucide-react";
...
export default function Dashboard() {
  ...
  const wardData = useMemo(() => {
    ...
    return Object.values(counts).sort(
      (a, b) => b.critical + b.moderate + b.low - (a.critical + a.moderate + a.low)
    );
  }, [data]);
  ...
  // (If you later implement real trend, keep the same hasData gate below.)
  ...
  if (loadingData || loadingStats || !stats || !data) {
  ...
  return (
    <div className="min-h-screen pt-16">
      <div className="mx-auto max-w-7xl px-4 py-8">
        ...
        {/* If no detections yet, show a nice empty state instead of blank charts */}
        {!hasData ? (
          <div className="mt-8">
            <EmptyState
              title="No detections yet"
              description="When the Raspberry Pi uploads the first pothole/barricade detections, charts and ward analytics will automatically appear here."
            />
          </div>
        ) : (
          <>
            ...
            <div className="mt-6 rounded-2xl border border-slate-800 bg-slate-900/50 p-6">
              ...
              <div className="mt-3 flex gap-6 justify-center">
                <div className="flex items-center gap-2">
                  <span className="h-0.5 w-5 rounded bg-orange-500" />
                  <span className="text-xs text-slate-400">New Detections</span>
                </div>
                <div className="flex items-center gap-2">
                  <span className="h-0.5 w-5 rounded bg-green-500" />
